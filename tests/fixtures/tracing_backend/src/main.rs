...
impl Worker {
    #[exec_time(backend = "tracing", name = "worker.trace")]
    fn run_trace(&self) -> i32 {
        1
    }

    #[exec_time(backend = "tracing", name = "worker.debug", level = "debug")]
    fn run_debug(&self) -> i32 {
        2
    }

    #[exec_time(backend = "tracing", name = "worker.slow", log_over = "1ms")]
    fn run_slow(&self) -> i32 {
        std::thread::sleep(Duration::from_millis(2));
        3
    }

    #[exec_time(backend = "tracing", name = "worker.suppressed", log_over = "50ms")]
    fn run_suppressed(&self) -> i32 {
        4
    }

    #[exec_time(backend = "tracing", name = "worker.warn", warn_over = "1ms")]
